...
public class CommentController {
    ...
    public String create(Model model, @PathVariable("id") Integer id, @Valid CommentForm commentForm, BindingResult bindingResult, Principal principal) {
        ...
        Comment comment = this.commentService.createComment(article, commentForm.getContent(), siteUser);
    ...
    @GetMapping("/vote/{id}")
    @ResponseBody
    public Integer commentVote(Principal principal, @PathVariable("id") Integer id) {
        Comment comment = this.commentService.getComment(id);
        SiteUser siteUser = this.userService.getUser(principal.getName());
        this.commentService.vote(comment, siteUser);

        Comment votedComment = this.commentService.getComment(id);
        Integer votedNum = votedComment.getVoter().size();
        return votedNum;
    }
